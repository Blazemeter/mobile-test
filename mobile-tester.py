...
def run(api_key, api_secret, name, silent):
  ...
  BASE_URL = '{}/api/v4'.format(os.environ.get('BZA_URL',
  ...
  response = requests.post(url='{}/tests'.format(BASE_URL), data=json.dumps(
      data), auth=auth, headers={"Content-Type": "application/json"})
  ...
  requests.post(
      url='{}/tests/{}/mobile-tests/start-uploads'.format(BASE_URL, test_id), auth=auth)
  got_upload_links = False
  while not got_upload_links:
    test_object = requests.get(
        url='{}/tests/{}'.format(BASE_URL, test_id), auth=auth).json()['result']
    mobile_test_configuration = test_object[
        'configuration']['plugins']['mobileTest']
  ...
  if not silent:
    application_path = raw_input(
        "Enter application path, hit `enter` for default ({}): ".format(app_path)).strip()
  ...
  while not app_upload_succeeded:
    requests.post(url='{}/tests/{}/mobile-tests/check-uploads'.format(BASE_URL,
                                                                      test_id), auth=auth, params={"fileType": "app"})
    test_object = requests.get(
        url='{}/tests/{}'.format(BASE_URL, test_id), auth=auth).json()['result']
    mobile_test_configuration = test_object[
        'configuration']['plugins']['mobileTest']
  ...
  while not test_package_upload_succeeded:
    requests.post(url='{}/tests/{}/mobile-tests/check-uploads'.format(BASE_URL,
                                                                      test_id), auth=auth, params={"fileType": "testPackage"})
    test_object = requests.get(
        url='{}/tests/{}'.format(BASE_URL, test_id), auth=auth).json()['result']
    mobile_test_configuration = test_object[
        'configuration']['plugins']['mobileTest']
  ...
  master_object = requests.post(
      url='{}/tests/{}/start'.format(BASE_URL, test_id), auth=auth).json()['result']
  ...
  while len(artifacts) == 0:
    artifacts = requests.get(url='{}/sessions/{}/mobile-test-artifacts'.format(
        BASE_URL, session_id), auth=auth).json()['result']
    print('.')
  ...
  results = [i for i in artifacts if i['type'] in [
      'SCREENSHOT', 'VIDEO', 'APPIUM_PYTHON_XML_OUTPUT']]

  # filter results for particular keys
  results = [{'type': ty['type'], 'name': ty['name'], 'url': ty['url']}
             for ty in results]
