...
def run(api_key, api_secret, name, silent):
  ...
  chosen_device = [device for device in devices if isinstance(
      device, dict) and device['platform'] == 'IOS' and device['os'] == "10.2"][0]
  ...
  logging.info("Chose a device. Deafult is %s(%s)",
               chosen_device['name'], chosen_device['os'])
